...
bool test_unicast_anonymous(LIBEFPIX_Identity* bob) {
    ...
    LIBEFPIX_Recv recv_msg = {0};
    bool decode_result = LIBEFPIX_decode(packet, *bob, &recv_msg, hash_check_and_relay, get_contact_from_alias, get_timestamp, get_age);
    bool pass = (decode_result && recv_msg.unknown && !recv_msg.broadcast && memcmp(recv_msg.message, anon_msg.message, LIBEFPIX_MESSAGE_SIZE)==0);
    printf("Result: %s\n", pass ? "PASS" : "FAIL");
    return pass;
}
bool test_broadcast_signed(LIBEFPIX_Identity* alice, LIBEFPIX_Identity* bob) {
    ...
    LIBEFPIX_Recv recv_msg = {0};
    bool decode_result = LIBEFPIX_decode(packet, *bob, &recv_msg, hash_check_and_relay, get_contact_from_alias, get_timestamp, get_age);
    bool pass = (decode_result && !recv_msg.unknown && recv_msg.broadcast && memcmp(recv_msg.broadcast_message, broadcast_msg.broadcast_message, LIBEFPIX_BROADCAST_MESSAGE_SIZE)==0 && memcmp(recv_msg.contact.sign_public_key, alice->sign_public_key, 32)==0);
    printf("Result: %s\n", pass ? "PASS" : "FAIL");
    return pass;
}
bool test_broadcast_anonymous(LIBEFPIX_Identity* bob) {
    ...
    LIBEFPIX_Recv recv_msg = {0};
    bool decode_result = LIBEFPIX_decode(packet, *bob, &recv_msg, hash_check_and_relay, get_contact_from_alias, get_timestamp, get_age);
    bool pass = (decode_result && recv_msg.unknown && recv_msg.broadcast && memcmp(recv_msg.broadcast_message, anon_broadcast.broadcast_message, LIBEFPIX_BROADCAST_MESSAGE_SIZE)==0);
    printf("Result: %s\n", pass ? "PASS" : "FAIL");
    return pass;
}
...
bool test_duplicate_message(LIBEFPIX_Identity* alice, LIBEFPIX_Identity* bob) {
    ...
    bool first_result = LIBEFPIX_decode(packet, *bob, &recv_msg1, hash_check_and_relay, get_contact_from_alias, get_timestamp, get_age);
    LIBEFPIX_Recv recv_msg2 = {0};
    bool second_result = LIBEFPIX_decode(packet, *bob, &recv_msg2, hash_check_and_relay, get_contact_from_alias, get_timestamp, get_age);
